...
6. Go to your Azure Portal, create a new `App Registration` 7. Search for and select `App Registration` 8. Create a new Registration named `your_registration`(unused value for deployment) 9. Note down the `Application (client) ID` as `your_client_id` 10. Select Certificates & secrets. 11. Select Client secrets, and then Select New client secret. 12. Provide a description of the secret, and a duration. 13. Select Add. 14. Note down the value of the secret as `your_client_secret`
7. Set your WebApp's environmental variables:
8. Go to the Azure Portal and navigate to your Webapp
9. In the left sidebar, go to `Settings` -> `Environment variables`.
10. Add the following App settings:
...
1. Deploy your application by calling `Deploy Webapp` action via 2. Arbitrary Pull Request 3. Manually on Github Webview under `Actions` Tab
