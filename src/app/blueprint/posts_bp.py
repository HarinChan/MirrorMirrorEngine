...
def _build_attachment_file_url(storage_key):
    ...
    public_api_base_url = (helper.get_env_variable('PUBLIC_API_BASE_URL','')).strip().rstrip('/')
    if public_api_base_url:
        return f"{public_api_base_url}{relative_url}"

    # Respect explicit env override; otherwise infer from the current request.
    force_https_env = helper.get_env_variable('FORCE_HTTPS_ATTACHMENT_URLS','') # will not throw if missing
    if force_https_env == '':
